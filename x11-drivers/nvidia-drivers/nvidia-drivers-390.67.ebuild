...
_nv_glvnd() {
	case "$1" in 
		GLVND) use glvnd && ! use system-glvnd && return 0 ;;
		NON_GLVND) ! use glvnd && ! use system-glvnd && return 0 ;;
	esac
	return 1
}
...
pkg_setup() {
	...
	if use kernel_FreeBSD; then
		...
		die "Could not determine proper NVIDIA kernel modulees source path in package."
	fi
}
